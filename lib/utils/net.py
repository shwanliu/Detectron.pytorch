...
def compute_iou(output, target, bbox_inside_weights, bbox_outside_weights,
    ...
    # old GIOU loss
    loss_log_miouk = ((1 - miouk) * iou_weights).sum(0) / output.size(0)
    # log-space GIOU loss
    loss_miouk = (((1 - miouk) * torch.exp(- miouk)) * iou_weights).sum(0) / output.size(0)

    return loss_iouk,loss_miouk,loss_log_miouk,iouk,miouk
